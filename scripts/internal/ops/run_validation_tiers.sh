...
run_help_tests() {
  ...
  PYTHONPATH=src python scripts/internal/workers/run_backtest_worker.py --help >/dev/null
}

run_smoke_unit_tests() {
  echo "[SMOKE][FAST] lightweight tests"
  PYTHONPATH=src python -m pytest     tests/test_generation_direct_mode.py     tests/test_v2_execution_hint_integration.py     -q
}
