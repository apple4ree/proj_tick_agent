#!/usr/bin/env python
"""
scripts/internal/workers/run_backtest_worker.py
----------------------------------------
CLI entry-point for the backtest worker.
...
./scripts/internal/workers/run_backtest_worker.sh

# With profile override (merged on top of the default config stack)
PYTHONPATH=src python scripts/internal/workers/run_backtest_worker.py --profile dev

# With an explicit override file (merged last, after profile)
PYTHONPATH=src python scripts/internal/workers/run_backtest_worker.py --config path/to/override.yaml
...
from utils.config import load_config, get_paths, get_workers
...
def main(argv: list[str] | None = None) -> None:
    ...
    if once:
        job = worker.run_once()
        if job is None:
            print("No backtest jobs in queue.")
            sys.exit(0)
        ...
        poll = workers.get("backtest_poll_interval", 5.0)
