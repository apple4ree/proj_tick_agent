...
#
# 사용법:
#   ./scripts/internal/run_local_stack.sh              # dev 프로필 (기본)
#   ./scripts/internal/run_local_stack.sh prod         # prod 프로필
set -euo pipefail
...
python scripts/internal/workers/run_generation_worker.py --profile "${PROFILE}" &
GEN_PID=$!
echo "[generation-worker] PID=${GEN_PID}"

# Start backtest worker in background
python scripts/internal/workers/run_backtest_worker.py --profile "${PROFILE}" &
BT_PID=$!
