...
./scripts/run_generation_worker.sh

# With profile override (merged on top of the default config stack)
PYTHONPATH=src python scripts/run_generation_worker.py --profile dev

# With an explicit override file (merged last, after profile)
PYTHONPATH=src python scripts/run_generation_worker.py --config path/to/override.yaml
