...
class TestUniverseDelayPropagation:
    ...
    def test_nonzero_latency_produces_nonzero_delay(self):
        """With latency_ms > 0, derived market_data_delay_ms should be non-zero
        and appear in the BacktestConfig used by the runner.

        This mirrors the config construction in
        backtest_strategy_universe.run_single_backtest().
        """
        ...
        latency_ms = 100.0
        derived_delay = latency_ms * 0.1  # 10.0
        ...
        latency_config = LatencyConfig(
            ...
            market_data_delay_ms=derived_delay,
            add_jitter=True,
        ...
        # Top-level field must match the derived value
        assert config.market_data_delay_ms == 10.0
        # Nested latency config must also match
        assert config.latency.market_data_delay_ms == 10.0

    def test_zero_latency_produces_zero_delay(self):
        """With latency_ms=0, market_data_delay_ms should be 0.0."""
        from evaluation_orchestration.layer7_validation.backtest_config import (
            BacktestConfig, LatencyConfig,
        )

        latency_ms = 0.0
        derived_delay = latency_ms * 0.1  # 0.0
    ...
    def test_universe_runner_receives_configured_delay(self):
        ...
        latency_ms = 500.0
        derived_delay = latency_ms * 0.1  # 50.0
